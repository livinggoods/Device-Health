...
public class AdminUsersJpaController implements Serializable {
	...
	public AdminUsers findByEmail(String email) {
		EntityManager em = getEntityManager();
		AdminUsers user = (AdminUsers) em.createNamedQuery("AdminUsers.findByEmail").setParameter("email", email)
				.getSingleResult();
		return user;
	}
	...
	public AdminUsers findByToken(String token) {
		EntityManager em = getEntityManager();
		AdminUsers user = (AdminUsers) em.createNamedQuery("AdminUsers.findByForgotToken")
				.setParameter("forgotToken", token).getSingleResult();
		return user;
	}
	...
	public AdminUsers findAdminUsers(Long id) {
		EntityManager em = getEntityManager();
		try {
			return em.find(AdminUsers.class, id);
		} finally {
			em.close();
		}
	}
